...
User.prototype.addSavedPost = function(id, title, rating) {
  let obj = { id: id, title: title, rating: rating };
...
User.prototype.removeSavedPost = function(id) {
  this.savedPosts = this.savedPosts.filter(elm => {
    return elm.id !== id;
  });
};
////////// PROBLEM 7 //////////
...
User.prototype.changePostRating = function(id, rating) {
  this.savedPosts = this.savedPosts.map(elm => {
    if (elm.id == id) elm.rating = rating;
    return elm;
  });
};
